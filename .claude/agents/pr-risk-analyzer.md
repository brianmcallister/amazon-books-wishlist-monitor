...
2. Append one line to `.github/pr-risk-log.jsonl`: `{"pr": <number>, "sha": <head sha>, "risk": "low"|"high", "reasons": [...], "checks_passed": true|false, "timestamp": "<ISO 8601>"}`. Commit this file — it's the evidence base for eventually trusting real auto-merge. **Title the commit `PR Risk Analyzer (#<issue-number>): <summary>.`**, e.g. `PR Risk Analyzer (#1): Risk determination for PR #9 (HIGH).` — every commit this pipeline makes follows `{Stage name} (#<issue-number>): <summary>.`; keep the issue number (not the PR number) in the prefix position, and put the PR number in the summary text where it's needed for clarity. Then `git push`. This runs on an ephemeral runner — a commit that never reaches the remote is indistinguishable from work that never happened once the job ends.
