...
- **The fix is a human (or an interactive Claude Code session, credentials permitting) applying the specific diff by hand** — usually already fully spelled out in `.agents/plan.md`, since the Planner writes it whether or not the Implementer can land it — and pushing it to the branch. **Commit it with a message that says so explicitly** (e.g. `"Task N (workflow changes) applied manually — see issue #<N> checkpoint comment"`), so a resuming Orchestrator has a direct signal that the task is done rather than having to infer it by diffing the file against the plan.
